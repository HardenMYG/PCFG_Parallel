...
// 异步GPU批量处理函数
void PriorityQueue::GenerateBatchOnGPU_Async(const vector<PTData>& pt_data_batch, 
                                           int total_passwords, 
    ...
    for (const auto& pt_data : pt_data_batch) {
        ...
        for (const string& value : pt_data.values) {
            ...
            int password_len = pt_data.prefix.length() + value.length() + 1;
    ...
    // 分配主机内存
    gpu_data.h_all_prefixes = new char[total_prefix_size];
    gpu_data.h_all_values = new char[total_values_size];
    gpu_data.h_output = new char[total_output_size];
    gpu_data.output_offsets = output_offsets;
    gpu_data.total_passwords = total_passwords;
    ...
    for (const auto& pt_data : pt_data_batch) {
        memcpy(gpu_data.h_all_prefixes + current_prefix_offset, pt_data.prefix.c_str(), pt_data.prefix.length());
        current_prefix_offset += pt_data.prefix.length();
        
        for (const string& value : pt_data.values) {
            memcpy(gpu_data.h_all_values + current_value_offset, value.c_str(), value.length());
            current_value_offset += value.length();
        }
    }
    
    // 异步分配设备内存
    cudaMallocAsync(&gpu_data.d_all_prefixes, total_prefix_size, gpu_data.stream);
    cudaMallocAsync(&gpu_data.d_all_values, total_values_size, gpu_data.stream);
    cudaMallocAsync(&gpu_data.d_output, total_output_size, gpu_data.stream);
    cudaMallocAsync(&gpu_data.d_prefix_offsets, pt_data_batch.size() * sizeof(int), gpu_data.stream);
    cudaMallocAsync(&gpu_data.d_prefix_lengths, pt_data_batch.size() * sizeof(int), gpu_data.stream);
    cudaMallocAsync(&gpu_data.d_value_offsets, total_passwords * sizeof(int), gpu_data.stream);
    cudaMallocAsync(&gpu_data.d_value_lengths, total_passwords * sizeof(int), gpu_data.stream);
    cudaMallocAsync(&gpu_data.d_pt_starts, pt_data_batch.size() * sizeof(int), gpu_data.stream);
    cudaMallocAsync(&gpu_data.d_pt_counts, pt_data_batch.size() * sizeof(int), gpu_data.stream);
    cudaMallocAsync(&gpu_data.d_output_offsets, total_passwords * sizeof(int), gpu_data.stream);
    
    // 异步数据传输到GPU
    cudaMemcpyAsync(gpu_data.d_all_prefixes, gpu_data.h_all_prefixes, total_prefix_size, 
                   cudaMemcpyHostToDevice, gpu_data.stream);
    cudaMemcpyAsync(gpu_data.d_all_values, gpu_data.h_all_values, total_values_size, 
                   cudaMemcpyHostToDevice, gpu_data.stream);
    cudaMemcpyAsync(gpu_data.d_prefix_offsets, prefix_offsets.data(), 
                   pt_data_batch.size() * sizeof(int), cudaMemcpyHostToDevice, gpu_data.stream);
    cudaMemcpyAsync(gpu_data.d_prefix_lengths, prefix_lengths.data(), 
                   pt_data_batch.size() * sizeof(int), cudaMemcpyHostToDevice, gpu_data.stream);
    cudaMemcpyAsync(gpu_data.d_value_offsets, value_offsets.data(), 
                   total_passwords * sizeof(int), cudaMemcpyHostToDevice, gpu_data.stream);
    cudaMemcpyAsync(gpu_data.d_value_lengths, value_lengths.data(), 
                   total_passwords * sizeof(int), cudaMemcpyHostToDevice, gpu_data.stream);
    cudaMemcpyAsync(gpu_data.d_pt_starts, pt_starts.data(), 
                   pt_data_batch.size() * sizeof(int), cudaMemcpyHostToDevice, gpu_data.stream);
    cudaMemcpyAsync(gpu_data.d_pt_counts, pt_counts.data(), 
                   pt_data_batch.size() * sizeof(int), cudaMemcpyHostToDevice, gpu_data.stream);
    cudaMemcpyAsync(gpu_data.d_output_offsets, output_offsets.data(), 
                   total_passwords * sizeof(int), cudaMemcpyHostToDevice, gpu_data.stream);
    
    // 异步启动GPU内核
    int blockSize = 256;
    int gridSize = (total_passwords + blockSize - 1) / blockSize;
    generate_batch_passwords_kernel<<<gridSize, blockSize, 0, gpu_data.stream>>>(
        gpu_data.d_all_prefixes, gpu_data.d_prefix_offsets, gpu_data.d_prefix_lengths,
        gpu_data.d_all_values, gpu_data.d_value_offsets, gpu_data.d_value_lengths,
        gpu_data.d_pt_starts, gpu_data.d_pt_counts, pt_data_batch.size(),
        gpu_data.d_output, gpu_data.d_output_offsets);
}

// 处理GPU结果
void PriorityQueue::ProcessGPUResults(AsyncGPUData& gpu_data) {
    // 确保所有操作完成
    cudaStreamSynchronize(gpu_data.stream);
    
    // 复制结果回主机
    cudaMemcpyAsync(gpu_data.h_output, gpu_data.d_output, gpu_data.total_passwords, 
                   cudaMemcpyDeviceToHost, gpu_data.stream);
    cudaStreamSynchronize(gpu_data.stream);
    
    // 添加到猜测列表
    for (int i = 0; i < gpu_data.total_passwords; i++) {
        guesses.push_back(string(gpu_data.h_output + gpu_data.output_offsets[i]));
    }
    total_guesses += gpu_data.total_passwords;
    
    // 清理资源（AsyncGPUData析构函数会处理）
}

// 优化的批量处理函数
void PriorityQueue::PopNextBatchCuda(int batch_size) {
    int actual_batch_size = min(batch_size, (int)priority.size());
    if (actual_batch_size == 0) return;
    
    static AsyncGPUData gpu_data;  // 静态变量保持GPU任务状态
    static bool gpu_task_in_progress = false;
    
    // 处理之前的GPU任务
    if (gpu_task_in_progress) {
        ProcessGPUResults(gpu_data);
        gpu_task_in_progress = false;
    }
    ...
    for (int batch_idx = 0; batch_idx < actual_batch_size; batch_idx++) {
        ...
        if (!pt_data.values.empty() && pt_data.values.size() < threshold) {
            ...
            cpu_work_passwords += pt_data.values.size();
    ...
    // 先处理CPU任务
    total_guesses += cpu_work_passwords;
    
    // 启动GPU批量处理（异步）
    if (!pt_data_batch.empty() && total_passwords > 0) {
        GenerateBatchOnGPU_Async(pt_data_batch, total_passwords, gpu_data);
        gpu_task_in_progress = true;
    }
    
    // 移除已处理的PT
    priority.erase(priority.begin(), priority.begin() + actual_batch_size);
    
    // 在GPU计算的同时，CPU处理新PT的合并工作
