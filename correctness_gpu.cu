...
using namespace chrono;
  ...
  //   nvcc -arch=sm_70 -std=c++11 -O2 correctness_gpu.cu md5.cu train.cu guess_gpu.cu -lcudart -o correctness_gpu 
...
int main()
{
    ...
    int history = 0;
    std::ofstream a("./output/results_guess.txt");
        ...
        if (curr_num > 1000000)
        {
            ...
            for (string pw : q.guesses)
            {
                if (test_set.find(pw) != test_set.end()) {
                    cracked+=1;
                    a << std::setw(8) << std::setfill('0') << hex << pw<<endl;
                }
