...
void PriorityQueue::PopNextBatch_mpi(int batch_size)
{
    ...
    for (int i = 0; i < actual_batch_size; i++) {
        pt_batch.push_back(priority[i]);
    }
    
    // 并行处理这批PT
    ProcessPTBatch_mpi(pt_batch);
...
// 处理PT批次的函数
void PriorityQueue::ProcessPTBatch_mpi(vector<PT>& pt_batch)
{
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    ...
    // 为每个进程分配PT
    int pts_per_process = batch_size / size;
    int remainder = batch_size % size;
    
    int start_pt, end_pt;
    if (rank < remainder) {
        start_pt = rank * (pts_per_process + 1);
        end_pt = start_pt + pts_per_process + 1;
    } else {
        start_pt = remainder * (pts_per_process + 1) + (rank - remainder) * pts_per_process;
        end_pt = start_pt + pts_per_process;
    }
    ...
    for (int pt_idx = start_pt; pt_idx < end_pt; pt_idx++) {
        PT& pt = pt_batch[pt_idx];
        
        // 为当前PT生成口令
        vector<string> pt_guesses;
        GenerateSinglePT_mpi(pt, pt_guesses);
        
        // 合并到本地猜测中
        local_guesses.insert(local_guesses.end(), pt_guesses.begin(), pt_guesses.end());
        local_total_guesses += pt_guesses.size();
    }
    ...
    // 同步所有进程的猜测计数
    int global_total = 0;
    MPI_Allreduce(&local_total_guesses, &global_total, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    total_guesses += global_total;
}

// 为单个PT生成口令的辅助函数
void PriorityQueue::GenerateSinglePT_mpi(PT& pt, vector<string>& pt_guesses)
{
    CalProb(pt);
    ...
    if (pt.content.size() == 1) {
        ...
        for (int i = 0; i < pt.max_indices[0]; i++) {
            pt_guesses.push_back(a->ordered_values[i]);
        }
    }
    else {
        string prefix;
        ...
        for (int i = 0; i < pt.max_indices[pt.content.size() - 1]; i++) {
            pt_guesses.push_back(prefix + a->ordered_values[i]);
        }
    }
}
